...
from mcb_traditional_rag.app import create_app
from mcb_traditional_rag.settings import parse_port
...
def expected_health() -> dict[str, object]:
    ...
    return next(item for item in responses if item["service"] == "traditional-rag")
...
def test_invalid_port_is_rejected(value: str) -> None:
    with pytest.raises(ValueError, match="MCB_TRADITIONAL_PORT"):
        parse_port(value)
